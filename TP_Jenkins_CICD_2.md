...
Deux choses sont nécessaires et complémentaires :

| Besoin | Ce que ça résout |
|--------|-----------------|
| Droits sur le groupe docker | Jenkins peut accéder à la socket `/var/run/docker.sock` |
| Binaire `docker` dans le conteneur | Jenkins peut envoyer des commandes au daemon |

**Étape 1 — Installer le binaire Docker CLI dans le conteneur Jenkins**
...
**Étape 2 — Donner les droits sur la socket Docker**
...
docker exec -u root jenkins groupadd -g 986 docker_host
...
```bash
echo "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIO9D5hPiP/889e3XFV3XTCOJoxqioSjZzeCa5bZpZqWf jenkins-deploy" >> ~/.ssh/authorized_keys
...
**Jenkins ne peut pas lancer de commandes Docker**

```bash
# Vérifier que la socket est bien montée
docker exec jenkins ls -la /var/run/docker.sock
# Vérifier les groupes de l'utilisateur jenkins
docker exec jenkins groups
```

**Le webhook ne déclenche pas la pipeline**

- Vérifiez que le port 8080 est bien ouvert dans le Security Group de l'EC2 Jenkins
- Consultez les logs du webhook dans GitHub/GitLab (onglet Recent Deliveries)
- Vérifiez les logs Jenkins dans **Manage Jenkins → System Log**

**SSH vers l'EC2 App échoue**

```bash
# Tester la connexion manuellement depuis le conteneur Jenkins
docker exec -u jenkins jenkins ssh -i /var/jenkins_home/.ssh/deploy_key -o StrictHostKeyChecking=no ubuntu@<IP_APP_EC2> "echo OK"
```

**L'image ne se pull pas sur l'EC2 App (ECR)**

```bash
# Vérifier que l'EC2 App a les droits ECR
aws ecr describe-repositories --region us-east-1
# Si erreur, vérifier le IAM Role attaché à l'instance
```
